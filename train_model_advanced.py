...
df["MACD_momentum"] = df["MACD"].diff().fillna(0)  # MACD acceleration
df["Price_velocity"] = df["Price_change"].diff().fillna(0)  # Price acceleration
...
for fb in future_bars_list:
    ...
    atr_threshold = df["ATR"] / df["Close"]  # Dynamic threshold
    returns_list.append(future_return > (atr_threshold * 0.75))

# Target: Majority vote across horizons
...
# Outlier removal (Robust scaling prevents outlier issues)
Q1 = X.quantile(0.25)
Q3 = X.quantile(0.75)
IQR = Q3 - Q1
...
majority_indices = y_train[y_train != minority_class].index

# Oversample minority class
oversample_indices = np.random.choice(minority_indices, len(majority_indices), replace=True)
balanced_indices = np.concatenate([majority_indices, oversample_indices])
...
# Class weight
target_counts = y_train_balanced.value_counts()
