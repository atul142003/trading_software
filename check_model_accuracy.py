...
print(f"    ✓ Indicators computed ({len(df)} valid records)")
...
print(f"    ✓ Positive class ratio: {y.mean()*100:.2f}%")
...
win_rate = (backtest_df['strategy_returns'].dropna() > 0).sum() / len(backtest_df['strategy_returns'].dropna()) * 100
...
print(f"Recommendation: {'Use with confidence' if accuracy > 0.60 else 'Needs improvement'}")
